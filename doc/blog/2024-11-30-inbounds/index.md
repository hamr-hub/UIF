...
当你在用浏览器访问 Google.com 时，一般来说浏览器会先检查你操作系统中是否设置了 `系统代理`，如果使用了，那么就会将请求先发送到 `系统代理` 所提供的地址，例如说：`127.0.0.1:10110`，这个地址就是由 UIF 提供的，所以 UIF 就能获取你想访问的网站，从而转发翻墙。
...
```bash
export http_proxy=http://127.0.0.1:10110
export https_proxy=http://127.0.0.1:10110
```

10110 是 UIF 默认 HTTP 入站的端口，你可以在入站列表里自行去查看或修改监听的端口。
